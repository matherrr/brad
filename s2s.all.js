...
/* 아래와 같이 if else문 바꾸기  ,, elif? elseif? 뭐가 맞지

    if(_searches.click_key){
        append_cookie(s2s_ck, _searches.click_key)
    }
    elif(!_searches.click_key){
        append_cookie(s2s_ck, _cookies.s2s_ck)
    }
...
//    if(s2s_params) {
//      cv = s2s_ck+'='+JSON.stringify(s2s_params)
//          +';max-age=900; domain=.samsung.com;path=/sec';
//      document.cookie = cv;
